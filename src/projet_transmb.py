...
#Argument Parser pour récupérer les arguments entrés par l'utilisateur.
...
argparser.add_argument('outfile', help="Path to input file (PDB)")
argparser.add_argument('--model',type =int, default=1, help="Modèle de la protéine étudié")
argparser.add_argument('--vector_nb',type =int, default=30, help="Nombre de vecteurs échantillonnés aléatoirement")
args= argparser.parse_args()

PDBFile=args.infile
OutputFile=args.outfile
PDBName=os.path.basename(args.infile)
PDBid=PDBName.split(".")[0]

#Lis le fichier PDB et le passe au programme DSSP, contient la valeur relative de la surface accessible (ASA) de chaque CA
p = PDBParser()
structure = p.get_structure(PDBid, PDBFile)
model_protein = structure[args.model-1]
dssp = DSSP(model_protein, PDBFile, dssp='mkdssp')

# Créer un dictionnaire DSSP à partir du fichier PDB, contient les Residus IDs de chaque CA.
dssp_tuple=dssp_dict_from_pdb_file(PDBFile)
dssp_dict = dssp_tuple[1]
id_tuple = [x[1] for x in dssp_dict] 
id=[y[1] for y in id_tuple] #Récupère les Résidus IDs dans une liste


df_DSSP = pd.DataFrame(dssp) #Créer un DataFrame à partir du fichier DSSP de départ.
df_column=[0,1,3]
df_DSSP = df_DSSP.iloc[:,df_column] #Garde seulement les IDs par défaut, le nom des acides aminés et les valeurs de relative ASA. 
df_DSSP.columns=['#','amino_acid','Relative ASA']
df_DSSP['#']=id  #Remplace les IDs par défaut du fichier DSSP par les Residus IDs stockées dans le dictionnaire DSSP.
#print (df.loc[[6]])
print(df_DSSP)
...
io = PDB.PDBIO
struct = p.get_structure(PDBid,PDBFile) 
model = structure[args.model-1] #Ne parcours que le modèle de la protéine souhaité, par défaut le 1er modèle.
...
vecteurs=sample_spherical(args.vector_nb,3)
...
io = PDB.PDBIO()
struct_transform = parser.get_structure(PDBid,PDBFile)
model_transform =struct_transform[args.model -1]
...
PDBTransform= os.path.join(OutputFile,'{0}_model{1}_transform.pdb'.format(PDBid, args.model))
io.save(PDBTransform)


#Lis le fichier contenant les nouvelles coordonnées avec le module Biotite.
atom_array = strucio.load_structure(PDBTransform)
...
PDBEdited=os.path.join(OutputFile,'{0}_model{1}_edited.pdb'.format(PDBid,args.model))
strucio.save_structure(PDBEdited, atom_array)
